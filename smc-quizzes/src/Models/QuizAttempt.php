...
class QuizAttempt extends Model
{
    ...
    {
        return config('smc-quizzes.table_names.quiz_attempts');
    }
    ...
    public function calculate_score($data = null): float
    {
        ...
        foreach ($quiz_questions_collection as $quiz_question) {
            ...
            $score += call_user_func_array(config('smc-quizzes.get_score_for_question_type')[$question->question_type_id], [$this, $quiz_question, $quiz_attempt_answers[$quiz_question->id] ?? [], $data]);
        }
        return $score;
    }
    ...
    public static function get_score_for_type_1_question(QuizAttempt $quizAttempt, QuizQuestion $quizQuestion, array | Collection $quizQuestionAnswers, $data = null): float
    {
        ...
        $correct_answer = ($question->correct_options())->first();
        $negative_marks = self::get_negative_marks_for_question($quiz, $quizQuestion);
        if (isset($correct_answer->id) && !empty($correct_answer)) {
            if (count($quizQuestionAnswers)) {
                //TODO
                //..
                foreach ($quizQuestionAnswers as $item) {
                    return $item->question_option_id == $correct_answer->id ? $quizQuestion->marks : -($negative_marks); // Return marks in case of correct answer else negative marks
                }
        ...
        return count($quizQuestionAnswers) ? (float) $quizQuestion->marks : 0; // In case of, no correct answer, if there is any answer then give full marks
    ...
    private function validateQuizQuestion(QuizAttempt $quizAttempt, QuizQuestion $quizQuestion, mixed $data = null): array
    {
        ...
        //echo print_r($answers) .PHP_EOL; exit();
        $questionType = $actualQuestion->question_type;
        $score = call_user_func_array(config('smc-quizzes.get_score_for_question_type')[$questionType->id], [$this, $quizQuestion, $answers ?? [], $data]);
        if ($score <= 0) {
            $isCorrect = false;
        }
        list($correctAnswer, $userAnswer) = config('smc-quizzes.render_answers_responses')[$questionType->id]($quizQuestion, $quizAttempt, $data);
        return [
            //'question' => $quizQuestion->question,
