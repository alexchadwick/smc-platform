...
```bash
composer require smc-dev/smc-quizzes
```
...
```bash
php artisan vendor:publish --provider="Quiz\Api\QuizServiceProvider"
```
...
```bash
php artisan db:seed --class=\\Quiz\\Api\\Database\\Seeders\\QuestionTypeSeeder
```
...
Add `CanAuthorQuiz` trait to your model, and you can get all the quizzes associated by calling the `quizzes` relation. 
