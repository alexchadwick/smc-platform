...
namespace Quiz\Api\Tests\Feature;
...
class QuizTest extends TestCase
{
    ...
    /** @test- */
    function quiz_multiple_choice_single_answer_few_wrong_answers()
    {
        $computer_science = Topic::factory()->create([
            'name' => 'Computer Science',
            'slug' => 'computer-science',
    ...
    function quiz_multi_user_attempts_multi_question_types_few_wrong_answers()
    {
        ...
        $response = $this->get('testing/quiz/'. $quiz->id);
        //$response->assertStatus(200);
        echo  PHP_EOL. (string) $response->content();

        //$response->assertSee('Test Laravel package isolated');

        //$view = $this->view('smc-quizzes.quiz', ['quiz' => $quiz]);
        $response->assertSee('question3gridRadio0');
        $response->assertSee('question3gridRadio1');
        $response->assertSee('question3gridRadio2');
        $response->assertSee('question3gridRadio3');
        $response->assertSee('quizQuestionAnswerInput[3]');

        $response = $this->post('testing/quiz/'. $quiz->id, [
            'quizQuestionAnswerInput[]' => [
                $quiz_question_one->id => $question_one_option_one->id,
                $quiz_question_two->id => [$question_two_option_one->id, $question_two_option_two->id],
                //$quiz_question_two->id => $question_two_option_one->id,
                //$quiz_question_three->id => $question_three_option_one->id,
                $quiz_question_three->id => $question_three_option_four->id,
            ],
        ]);
